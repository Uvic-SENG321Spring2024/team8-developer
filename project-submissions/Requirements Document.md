...
The system being improved is the employee management system at Banter Ice Cream. The current system uses two applications, Homebase and Basecamp, for scheduling, recording time worked, and communication. Employees at Banter use Homebase at least once per week because it is used to check shift schedules, swap shifts, and record time worked. Having two applications results in Basecamp is being used less frequently, leading employees to miss announcements about new ice cream flavours and messages about scheduled deliveries of ice cream between locations. Employees missing ice cream flavour announcements has lowered customer satisfaction, because information customer's recieve about updated ice flavours is not accurate. Employees missing messages about scheduled deliveries lowers efficiency as the employees are not prepared to unload the delivered ice cream and results in each delivery driver spending longer on their delivery. It is also difficult to onboard new employees, as learning two applications takes more time than learning one application. Finally, the current scheduling system cannot summarize hours worked for each employee the way Banter Ice Cream expects. Payroll is time consuming and prone to manual error because bookkeepers need to manually calculated regular, holiday, overtime, sick days, and vacation days for each employee.

The new system will resolve these issues by being a single platform, that combines the functionality of Homebase and Basecamp. It will improve on the scheduling functionality of Homebase and include the communication functionality of Basecamp. The new system will also store information about Banter Ice Cream, including onboarding materials and ice cream recipes, so they are available to each employee with access. The payroll errors will also be reduced by summarizing hours according to the client's requests. The new system shall result in more engaged employees, more satisfied customers, fewer payroll errors, and easier onboarding for new employees.
